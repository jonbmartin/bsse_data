...
if newsim:
    ...
    rf_pulse_base = np.ones([1, pulse_samples], dtype=np.complex) * 4258*2*np.pi*0.05  # gauss

    pulse_dur = np.size(rf_pulse_base)*dt_ex

    # design our BS pulse:
    rfp_bs, _, _ = rf.dz_bssel_rf(dt=dt_ex, tb=tb, ndes=128, ptype='ex', flip=np.pi / 2, pbw=pbw,
                            pbc=[pbc], d1e=d1e, d2e=d2e,
                            rampfilt=True, bs_offset=5000)
    constant_bs_pulse = rfp_bs[:,800] # pick a point in the middle of the pulse
    constant_bs_pulse *= 1.6 * 4258*2*np.pi  # radians
    # pl.LinePlot(rfp_bs)
    ...
    for ii in range(0, n_ex):
        output = mr.bloch_forward(np.real(output), constant_bs_pulse, f0, t1, t2, dt)
        decay[:, :, ii] = output

    # 180 trainconstant_bs_pulse*100
