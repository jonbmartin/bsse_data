...
The passband of the pulse can be shifted by adjusting the frequency content of the frequency-selective pulse, 
...
simulated and experimental pulses produce selective excitation with the specified profile characteristics.
Experimental profiles closely match simulated patterns. 
Multiphoton resonances produced by the pulses are characterized and predicted. \\
\textbf{Conclusion}
...
% ADD: section that discusses the various tradeoffs in the design. duration for ripple for BS offset etc etc

% ADD: discussion of the challenges in pulse performance at very low B_1^+. This design algorithm does start to fall apart a bit...is that true for SLR in general? May be because of reliance on SLR algorithm, which in general assumes a linear frequency gradient - this assumption is worst at low B1 
