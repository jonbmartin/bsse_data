...
pbc_max = 8  # actually sims down to 1 G below (7G) # originally 8
pbc_step = 0.25
...
if newsim:
    for pbc in list(pbc_list):
        for pbw in list(pbw_list):
            ...
            while bsse_dur < rfse_dur:
                ...
                bsse_off +=40 # was 20, 40 for quicker computation
    ...
    sio.savemat('rfse_comp_full_b1simto10_TB6_widerw.mat',dic)
...
data = sio.loadmat('rfse_comp_full_b1simto10_TB6_widerw.mat')
y = len(pbw_list)
x = len(pbc_list)
...
# marker symbol
im0 = axs[0, 0].imshow(bsse_rmse_v[0:max_pbc_index, 0:max_pbw_index], cmap ='jet', vmin=0, vmax=0.02, extent=extent,aspect=aspect)
axs[0, 0].set_title("BSSE RMSE")
fig.colorbar(im0, ax=axs[0, 0])
axs[0,0].set_ylabel('PBC (G)')

im1 = axs[0, 1].imshow(bsse_stoprip_v[0:max_pbc_index, 0:max_pbw_index], cmap ='jet', vmin=0, vmax=0.05, extent=extent, aspect=aspect)
