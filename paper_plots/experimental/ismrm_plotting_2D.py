...
grid[4].title.set_text('g) $B_1^+$ map (mT)')
grid[0].title.set_text('c) Base Pulse')
grid[1].title.set_text(r'd) PBC $\rightarrow$ 0.11 mT')
grid[2].title.set_text(r'e) FA $\rightarrow$ 45$^{\circ}$')
grid[3].title.set_text(r'f) TB $\rightarrow$ 1.5')
grid[2].set_xlabel('relative z position (mm)')
...
# axs[1,0].legend([r'TB=4',r'TB=1.5'],loc='upper left')
...
# axs[0,0].legend([r'FA=90$^\circ$',r'FA=45$^\circ$'],loc='upper left')
